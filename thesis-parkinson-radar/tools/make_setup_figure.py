...
fig = plt.figure(figsize=(15.0, 8.6), dpi=100)
...
ax.text(1.65, 1.97, "Acquisition geometry: two radars watching the same walk",
        ha="center", va="top", fontsize=16.5, fontweight="bold", color=INK)
ax.text(1.65, 1.83, "one node aimed at the trunk, two aimed at the feet, 40° beamwidth",
        ha="center", va="top", fontsize=12, color=SUBINK, style="italic")
...
for xa, xb, lab, col in [(-0.89, 0.0, "1 m standoff", "#7c8a99"),
    ...
    ax.text((xa + xb) / 2, -0.40, lab, ha="center", va="center",
...
ax.text(-0.89, 1.30, "TORSO radar\n1.00 m high", ha="center", va="center",
        fontsize=12.5, fontweight="bold", color=TORSO_C, linespacing=1.5)
ax.text(-0.89, 0.45, "FOOT radars ×2\n0.15 m high", ha="center", va="center",
        fontsize=12.5, fontweight="bold", color=FOOT_C, linespacing=1.5)
...
ax.annotate("", xy=(2.62, 1.02), xytext=(2.02, 1.02),
            arrowprops=dict(arrowstyle="-|>", color="#1f7a55", lw=3.2,
                            mutation_scale=22))
ax.text(2.32, 1.12, "walk", ha="center", va="bottom", fontsize=13,
...
ax.text(3.32, 1.30,
        ...
        ha="left", va="center", fontsize=12.5, color=TORSO_C, linespacing=1.5,
        bbox=dict(boxstyle="round,pad=0.42", fc="white", ec=TORSO_C, lw=1.3),
        zorder=7)
ax.text(3.32, 0.34,
        "feet peak at 3 to 4 m/s\n$\\rightarrow$ shift up to about 500 Hz",
        ha="left", va="center", fontsize=12.5, color=FOOT_C, linespacing=1.5,
        bbox=dict(boxstyle="round,pad=0.42", fc="white", ec=FOOT_C, lw=1.3),
        zorder=7)
...
bx.text(50, 96, "From radio wave to the two pictures we analyse",
        ha="center", va="top", fontsize=16, fontweight="bold", color=INK)

steps = [
    ("Transmit\nand reflect",
     "a 23 GHz chirp sweeps\n1.4 GHz, one every 625 µs"),
    ("Remove\nstill objects",
     "a 10 Hz high-pass drops\nwalls and furniture"),
    ("Fourier transform\nin short slices",
     "a 50 ms window slid along\nthe recording"),
    ("Spectrogram",
     "320 speed slots covering\n±800 Hz, one per 5 Hz"),
    ("Brightness rescaled,\ntwo channels kept",
     "one aimed at the feet,\none aimed at the trunk"),
]
...
for i, (title, sub) in enumerate(steps):
    ...
    bx.text(x + w / 2, y0 + h - 9, f"{i+1}", ha="center", va="center",
            fontsize=11, fontweight="bold", color="white",
            bbox=dict(boxstyle="circle,pad=0.30",
                      fc=ACCENT if last else "#7d99b5", ec="none"))
    bx.text(x + w / 2, y0 + h - 26, title, ha="center", va="center",
            fontsize=14, fontweight="bold", color=ACCENT if last else INK,
            linespacing=1.35)
    bx.text(x + w / 2, y0 + 12, sub, ha="center", va="center",
            fontsize=11.5, color=SUBINK, style="italic", linespacing=1.45)
    if not last:
...
bx.text(50, 4.5,
        ...
        ha="center", va="center", fontsize=12, color=SUBINK, style="italic")
