...
# These canvases are 11 to 13.6 in wide but print at \textwidth (16 cm), a
# 0.45-0.55x reduction, so every size is chosen for the printed page:
...
STAGES = [
    ("Recording", "one .mat file", "320 x 12,000", "7.5 s of walking", V.INK_2),
...
def band(items, y, h, label, sublabel):
    n = len(items)
    gap, x0, x1 = 2.4, 13.5, 99.0
    w = (x1 - x0 - gap * (n - 1)) / n
    ax.text(0.6, y + h / 2 + 4.0, label, fontsize=17, fontweight="bold",
            color=V.INK, va="bottom")
    ax.text(0.6, y + h / 2 + 2.0, sublabel, fontsize=14, color=V.INK_3, va="top",
            linespacing=1.35)
    for i, (t, s, shape, foot, col) in enumerate(items):
        ...
        ax.text(x + w / 2, y + h - 6.5, t, ha="center", va="center",
                fontsize=16.5, fontweight="bold", color=V.INK)
        ax.text(x + w / 2, y + h - 14.5, s, ha="center", va="center",
                fontsize=15, color=V.INK_2, style="italic")
        ax.text(x + w / 2, y + 9.5, shape, ha="center", va="center",
                fontsize=15, color=col, family="monospace", fontweight="bold")
        ax.text(x + w / 2, y + 3.8, foot, ha="center", va="center",
                fontsize=14.5, color=V.INK_3)
        if i < n - 1:
...
band(STAGES, 60, 33, "Built once", "348 recordings\nin; 1,673\nwindows out")
band(AFTER, 12, 33, "Per fold", "runs 58 times,\nonce per\nheld-out subject")
...
ax.add_patch(FancyArrow(16.5, BUSY, 0, -4.6, width=0.35, head_width=1.9,
                        ...
                        facecolor=V.INK_3, edgecolor="none"))
ax.text(55, BUSY + 1.6, "the cache is written once and read by all 58 folds",
        ha="center", fontsize=14, color=V.INK_3, style="italic")
...
fig, (ax, axw) = plt.subplots(2, 1, figsize=(12.2, 6.2), sharex=True,
...
V.title(ax, "Every recording is cut into equal-length windows",
        ...
        f"each starting 1.5 s after the last", sub_size=14.5, pad=32)
V.note(ax, t[0] + 0.35, dop[-1] * 0.72, "positive = towards the node",
...
ax.annotate(f"{_lo:.1f}x", (0.0 - 0.10, _lo), textcoords="offset points",
            xytext=(-52, 0), fontsize=14.5, color=V.INK_2, va="center",
            arrowprops=dict(arrowstyle="-", color=V.RULE, lw=1))
...
V.note(a, 0.5, top * 0.92, "straddles\nthe turn", ha="center", color=V.PD,
       fontsize=14.5)
V.note(a, 0.14, top * 0.95, "walking away", ha="center", fontsize=14.5)
V.note(a, 0.86, top * 0.95, "walking back", ha="center", fontsize=14.5)
...
for x, col in [(0.1, V.ACCENT), (0.2, V.PD)]:
    ...
    a.annotate(f"threshold {x}\nkeeps {k:.0f}% of windows", (x, k),
               textcoords="offset points", xytext=(16, 4), fontsize=14.5,
               color=V.INK_2, va="center")
a.set_xlabel("how much of the turn to exclude")
a.set_ylabel("% of windows kept")
a.set_ylim(0, 105)
V.title(a, "What excluding the turn costs", pad=32)
...
a.annotate("epoch 0 selected\nand never beaten", (0, 0.5), textcoords="offset points",
           xytext=(14, 44), fontsize=14, color=V.INK_2,
...
a.set_xlabel("epoch")
V.title(a, "Before", "validation set is one subject, so the AUC is\nundefined "
                     "and was replaced by a constant", sub_size=14, pad=52)
...
a.annotate(f"best epoch {best}\nAUC {real[best]:.3f}", (best, real[best]),
           textcoords="offset points", xytext=(-16, -52), fontsize=14,
           color=V.INK_2, ha="center",
           arrowprops=dict(arrowstyle="-|>", color=V.INK_3, lw=1.2))
a.set_xlabel("epoch")
V.title(a, "After", "validation set is 8 stratified subjects,\nso the AUC is real",
        sub_size=14, pad=52)
a.axhline(0.5, color=V.RULE, lw=1.2, zorder=0)
V.note(a, 0.4, 0.512, "chance", ha="left", fontsize=14)
...
], loc="lower center", ncol=2, fontsize=13.5, bbox_to_anchor=(0.5, -0.16))
...
for name, n, col, sub in segs:
    ...
    ax.text(x + w / 2, 60, name, ha="center", va="center", fontsize=15.5,
            fontweight="bold", color=V.INK)
    ax.text(x + w / 2, 49, f"{n} subject{'s' if n > 1 else ''}", ha="center",
            va="center", fontsize=14, color=col, fontweight="bold")
    ax.text(x + w / 2, 33, sub, ha="center", va="top", fontsize=13.5, color=V.INK_2)
    x += w + 1.1
ax.text(2, 88, "One fold, repeated 58 times", fontsize=18, fontweight="bold", color=V.INK)
...
for yi, (lab, v, col, tag) in zip(y, rows):
    ...
    ax.text(v * 1.35, yi, f"{v:,}", va="center", fontsize=14.5,
            fontweight="bold", color=V.INK)
...
ax.annotate(f"{len(m):,} training windows", (len(m), len(rows) - 0.35),
            textcoords="offset points", xytext=(9, 0), fontsize=14,
