#!/usr/bin/env python3
"""Render the Grad-CAM example gallery for resnet18_fc_v2.

run_gradcam.py computed per-window attention for both image models but only
rendered the example gallery for smallcnn_v2. This script picks the same
2x4 layout (confident correct calls on top, confident mistakes below) from
the stored per-window summary and renders it for the ResNet-18 transfer
model, recomputing the maps only for the folds the gallery needs.
"""
import sys
...
out = G.FIGS / "gradcam_gallery_resnet.png"
